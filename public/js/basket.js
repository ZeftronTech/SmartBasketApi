...
function fetchData(event) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            else updateBasketsGrid(data.data);
        },
        error: function (data) {
            console.log("Error");
            console.log(data);
...
function updateBasketsGrid(data_images) {
    ...
    for (j = 0; j < basket_keys.length; j++) {
        ...
        for (i = 0; i < basket.length; i++) {
            ...
            html += "<a class='grid' href='"+basket[i].url+"'><figure> <img src='" + basket[i].url + "' width='350' height='300' alt='Image' class='grid'> <figcaption> " + date + "</figcaption> </figure> </a>"
        }
